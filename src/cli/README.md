...
Additionally, the user must be able to connect to a browser session remotely on Browserbase to be able to know if their function works. Some functions don't define browser settings, but some functions will define browser settings like `advancedStealth: true` for example. The system needs to be able to support this "on incoming `/invoke` call, create a default (or customized, depending on the function configuration) browser session" which will get passed in as `session.id` and `session.connectUrl`. To connect to a browser session, the user must somehow specify their `projectId` and `apiKey`. Let's have the user put these in a `.env` file and then the `cli` looks for `.env` with `import "dotenv/config"`. This should use the Browserbase SDK under the hood.
